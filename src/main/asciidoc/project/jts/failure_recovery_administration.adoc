...
Recovery responsibility is delegated to link:#recovery-manager[The Recovery Manager] . Recovery after failure requires that information about the transaction and the resources involved survives the failure and is accessible afterward: this information is held in the `ActionStore`, which is part of the `ObjectStore`.
...
This can be used without modification, except possibly the debug tracing fields, as shown in <<jts_recovery_manager_output>>.

[[jts_recovery_manager_output]]
== Output
...
[source,xml]
----
include::{projectSourceDir}/jts/extras/default_recovery_extension_settings.xml[]
----
...
[source,xml]
----
include::{projectSourceDir}/jts/extras/expiry_scanner_properties.xml[]
----
