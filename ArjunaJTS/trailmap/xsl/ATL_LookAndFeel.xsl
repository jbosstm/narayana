...
function emailThisPage()
{
    ...
    <xsl:template match="links[@include]" mode="header">
        <xsl:apply-templates select="document(@include)" mode="header"/>
    </xsl:template>

    <xsl:template match="links[position()>1][@include]" mode="nav">
    ...
    <xsl:template match="abslink" mode="header">
    ...
    <xsl:template match="imgnews" mode="header">
        <xsl:message terminate="yes">
            "imgnews" in header not supported
        </xsl:message>
    </xsl:template>
