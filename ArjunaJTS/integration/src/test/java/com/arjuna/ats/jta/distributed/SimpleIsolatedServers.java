...
//                    }
    ...
    private void doRecursiveTransactionalWork2(int startingTimeout, List<String> nodesToFlowTo, boolean commit, boolean rollbackOnlyOnLastNode) throws Exception {
        ...
        // Align the local state with the returning state of the transaction from the subordinate
        switch (dataReturnedFromRemoteServer.getTransactionState()) {
            case Status.STATUS_MARKED_ROLLBACK:
            case Status.STATUS_ROLLEDBACK:
            case Status.STATUS_ROLLING_BACK:
    ...
    private DataReturnedFromRemoteServer performTransactionalWork(List<String> nodesToFlowTo, int remainingTimeout, Xid toMigrate,
        ...
        if (!nodesToFlowTo.isEmpty()) {
            ...
            /*
             * Only propagate active transactions - this may be inactive through user code
             * (rollback/setRollbackOnly) or it may be inactive due to the transaction reaper
             */
            if (status == Status.STATUS_ACTIVE) {
                ...
                /*
                 * Create a proxy for the new server if necessary, this can orphan the remote server,
                 * but XA recovery will handle that on the remote server.
                 * The alternative is to always create a proxy but this is a performance drain and
                 * will result in multiple subordinate transactions and performance issues
                 */
    ...
    /**
     * This is the transactional data the transport needs to return from remote instances.
     */
