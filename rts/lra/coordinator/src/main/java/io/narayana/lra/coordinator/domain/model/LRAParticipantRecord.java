...
public class LRAParticipantRecord extends AbstractRecord implements Comparable<AbstractRecord> {
    ...
    private int runPostLRAActions() {
        ...
        if (lraStatus == LRAStatus.Cancelling) {
            report = isFailed();
            lraStatus = report ? LRAStatus.FailedToCancel : LRAStatus.Cancelled; // this participant is the last one to finish
        } else if (lraStatus == LRAStatus.Closing) {
            report = isFailed();
            lraStatus = report ? LRAStatus.FailedToClose : LRAStatus.Closed; // this participant is the last one to finish
        }
    ...
    private int reportFailure(String failureReason) {
        if (status == ParticipantStatus.FailedToCompensate) {
            ...
            return reportFailure(true, completeURI, failureReason);
        }
    }
