...
function init_test_options {
    [ $NARAYANA_CURRENT_VERSION ] || NARAYANA_CURRENT_VERSION="5.1.1.Final-SNAPSHOT"
...
function build_as {
  ...
  JAVA_OPTS="$JAVA_OPTS -Xms1303m -Xmx1303m -XX:MaxPermSize=512m" ./build.sh -f testsuite/integration/basic/pom.xml
  [ $? = 0 ] || fatal "AS basic integration tests failed"
...
function blacktie {
  ...
  unzip ${WORKSPACE}/blacktie/wildfly-blacktie/build/target/wildfly-blacktie-build-5.1.1.Final-SNAPSHOT-bin.zip -d $PWD/blacktie/wildfly-${WILDFLY_MASTER_VERSION}
